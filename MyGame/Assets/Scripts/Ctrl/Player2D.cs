...
public class Player2D : MonoBehaviour
{
    ...
    public float pushWallImpulse;// 蹬墙冲力
    private bool leftStickWall;
    private bool rightStickWall;
    private float downTime;// 按下时间
    public float lessGravityScale;// 更小重力
    public bool isJump;// 是否跳跃
    public bool isJumpOnWall;
    ...
    private void FixedUpdate()
    {
        ...
        leftStickWall=Physics2D.OverlapCircle(wallCheckLeft.position,0.1f,wallLeft);
        rightStickWall=Physics2D.OverlapCircle(wallCheckRight.position,0.1f,wallRight);
    ...
    private void Update() {
        ...
        wallGrab=isWall&&Input.GetKey(KeyCode.LeftShift);
        ...
        if(Input.GetKeyDown(KeyCode.J)&&canSprint){// 冲刺
        ...
        // 没有在滑翔状态时，点击右键可以进行滑翔
        if(Input.GetKeyDown(KeyCode.K)&&isGlide&&canGlide){

            GlidingState();
            ctrl.audioManager.OpenUmAudio();// 播放开伞音效
        }
        if(Input.GetKey(KeyCode.K)&&isGround&&canGlide){
            CancelGliding();
        }
        if(Input.GetKeyUp(KeyCode.K)&&canGlide){
        ...
        */
        ...
        /*
        if((Input.GetKeyDown(KeyCode.A)||Input.GetKeyDown(KeyCode.D))&&isGround){
            
            ctrl.audioManager.RunAudio();// 播放走路音效
        }
        if(!isGround){
    ...
    void VerticalMove(){// 竖直方向移动
        ...
        _rigidbody.velocity=new Vector2(_rigidbody.velocity.x,verticalMove*moveSpeed);
    }
    ...
    void Jumping(){// 跳跃
        ...
        if(!isGround&&!isWall){// 不在地面，无法进行跳跃
            jumpPressed=false;
        }
        /*
        if(jumpPressed&&isWall){// 在墙上且按下空格时跳跃
            isJump=true;

            isJumpOnWall=true;

            if(leftStickWall){
                _rigidbody.velocity=new Vector2(pushWallImpulse,jumpSpeed);
            }
            if(rightStickWall){
                _rigidbody.velocity=new Vector2(-pushWallImpulse,jumpSpeed);
            }
            ...
            ctrl.audioManager.JumpAudio();
        }
        */
        if(jumpPressed&&isGround){// 在地面且按下空格时跳跃
            isJump=true;
        ...
        if(isSprint){
            if(sprintTimeLeft>0){
                ...
                /*if(_rigidbody.velocity.y>0&&!isGround){
                    _rigidbody.velocity=new Vector2(sprintSpeed*gameObject.transform.localScale.x ,jumpSpeed);
                }*/
                _rigidbody.velocity=new Vector2(sprintSpeed*gameObject.transform.localScale.x ,_rigidbody.velocity.y);
    ...
    void CancelGliding(){
        ...
        isGlide=false;// 若松开右键，则滑翔状态取消
    }

    void SwitchAnim(){// 切换动画
        animator.SetFloat("Run",Mathf.Abs(horizontalMove));// 移动时播放跑步动画
    ...
    void DieEffect(){
        dieEffect = Instantiate(die,transform.position,transform.rotation);
        
    }
    private void OnTriggerEnter2D(Collider2D other) {
        if(other.tag=="DieTerrain"){
